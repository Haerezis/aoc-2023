  ...
  (-1..1).each do |line_inc|
    (-1..1).each do |column_inc|
      ...
      if 0 <= lookup_line && lookup_line < line_count && 0 <= lookup_column && lookup_column < column_count
        ...
        if is_integer?(cell)
          integer_positions.push([cell, lookup_line, lookup_column, [line, column]])
        end
      end
    end
  end
...
integer_positions.each do |v, line, column, sym_positions|
...
integers_per_symbol_positions = {}
whole_integer_positions.each do |v, line, column, sym_positions|
  lookup_column = column
  while lookup_column < column_count && is_integer?(matrix[line][lookup_column + 1])
    lookup_column += 1
  end

  integers_per_symbol_positions[sym_positions] ||= []
  integers_per_symbol_positions[sym_positions].push([
    lines[line][column..lookup_column].to_i,
    line,
    column,
    lookup_column
  ])
...
total = 0
integers_per_symbol_positions.values.each do |integers|
  if integers.count == 2 
    total += integers.map(&:first).inject(:*)
  end
end
